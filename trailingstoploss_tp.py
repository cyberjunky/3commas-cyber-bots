            ...
            if deal_strategy == "short":
                ...
                if not existing_deal and float(deal["actual_profit_percentage"]) >= activation_percentage:
                    ...
                    if current_stoploss_percentage != 0.0:
                        ...
                        update_short_deal(thebot, deal, existing_deal)
                    else:
...
def new_short_deal(thebot, deal):
    """New short deal to activate SL on"""
    ...
    sold_average_price = float(deal["sold_average_price"])
    sl_price = sold_average_price - (sold_average_price * ((initial_stoploss_percentage / 100.0)
                                       + ((activation_diff / 100.0) * sl_increment_factor))
                                       )

    logger.debug(
        f"{pair}/{deal_id}: SL price {sl_price} calculated based on sold "
        f"price {sold_average_price}, initial SL of {initial_stoploss_percentage}, "
    ...
    if base_price_sl_percentage != 0.00:
        ...
        average_price_sl_percentage = round(
            100.0 - ((sl_price / sold_average_price) * 100.0),
            2
        )

        logger.debug(
            f"{pair}/{deal_id}: average SL of {average_price_sl_percentage}% "
            f"calculated based on average price {sold_average_price} and "
            f"SL price {sl_price}."
        )
...
def update_short_deal(thebot, deal, existing_deal):
    """Update short deal and increase SL (Trailing SL) if profit has increased."""
    ...
    if actual_profit_percentage > last_profit_percentage:
        ...
        if new_base_price_sl_percentage != 0.00:
            ...
            current_average_price_sl_percentage = round(
                100.0 - ((current_base_sl_price / sold_average_price) * 100.0),
                2
            )
            new_average_price_sl_percentage = round(
                100.0 - ((new_base_sl_price / sold_average_price) * 100.0),
                2
            )

            logger.debug(
                f"{pair}/{deal_id}: average SL {current_average_price_sl_percentage}% "
                f"based on base sl price {current_base_sl_price} and sold price "
                f"{sold_average_price}. New average SL {new_average_price_sl_percentage}% "
                f"based on new base sl price {new_base_sl_price} and sold price "
                f"{sold_average_price}."
            )
